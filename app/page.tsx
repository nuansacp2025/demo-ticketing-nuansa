...
function attemptLogin(event: React.FormEvent<HTMLFormElement>) {
  event.preventDefault();
  const formData = new FormData(event.target as HTMLFormElement);
  const email = formData.get("email") as string;
  const ticketId = formData.get("ticketId") as string;
  const response = login(email, ticketId);
  if (response.status) {
    setTimeout(() => {redirect(`/ticket/${response.address}`)}, 1000);
  }
}
    ...
    <div className="flex h-screen flex-col justify-center px-6">
      <div className="sm:mx-auto sm:w-full sm:max-w-sm">
        <form className="space-y-6" onSubmit={attemptLogin}>
          ...
          <div>
            <div className="flex items-center justify-between">
              <label htmlFor="ticketId" className="block text-sm/6 font-medium text-[#1A7F7D]">Ticket ID</label>
            </div>
            <div className="mt-2">
              <input type="text" name="ticketId" id="ticketId" autoComplete="ticketId" required className="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-[#1A7F7D] sm:text-sm/6"/>
            </div>
          </div>
        ...
        <p className="mt-10 text-center text-sm/6">
          <a href="#" className="font-semibold text-[#1A7F7D]/75 hover:text-[#1A7F7D]">How do I find my ticket number?</a>
        </p>
      </div>
    </div>
  );
